...
#### Task AUTH-001-04: Run database migration for passkey table
...
**Files:**
- Create: Database migration (auto-generated by better-auth)
...
**Step 2: Verify migration created passkey table**

Run: Check database schema for `passkey` table with columns:
...
```bash
# Apply migration (command depends on your migration tool)
pnpm db:migrate
```
...
**Files:**
- Test: `klard-mobile/src/lib/__tests__/auth-client.test.ts` - Add cookie prefix test
...
- Test: `klard-auth/tests/integration/well-known.test.ts`
...
- [ ] Integration test verifies file accessibility
