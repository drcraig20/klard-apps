...
**Goal:** Implement a reusable Modal/Dialog component for both Web (shadcn/ui Dialog) and Mobile (React Native Modal) platforms with consistent API, controllable open state, sized content, optional header/description/footer, overlay behavior, and close handling consistent with Klard design and accessibility standards.

**Architecture:** The Modal component wraps platform-specific implementations (shadcn/ui Dialog for web, React Native Modal for mobile) behind a unified props interface. Web uses Radix UI primitives via shadcn/ui with size variants, close-on-overlay toggle, and footer slot. Mobile uses native Modal with KeyboardAvoidingView, SafeAreaInsets, overlay tap-to-close (configurable), and optional footer slot. Logic (open state, handlers) is injected; components remain purely presentational.

**Tech Stack:**
- Web: React 19, shadcn/ui Dialog (Radix UI), Tailwind CSS 4, CVA, class-variance-authority
- Mobile: React Native Modal, expo-haptics, react-native-safe-area-context, @expo/vector-icons (Ionicons), KeyboardAvoidingView, Platform
...
|-----------|-----|-----|-----|-----|-----|
| Modal (Web) | Renders dialog shell with sizing and close behavior only | Sizes configurable via data map; overlay behavior via prop; extensible via className | Handles all DialogContent variants; accepts any children/footer/title | Single ModalProps interface with optional props | Depends on shadcn Dialog + injected handlers + ReactNode abstraction |
| Modal (Mobile) | Presents overlay shell with keyboard/safe area handling only | Overlay closing and sizes configurable; extensible via style | Works with any content/onClose; handles all Modal animations | Single ModalProps interface with optional props | Depends on RN/Expo primitives + injected handlers + ReactNode abstraction |
...
- **open/closed:** Controlled via `open` prop
- **Focus trapping:** Handled by shadcn Dialog (web); native modal behavior (mobile)
- **Overlay click-block:** When `closeOnOverlay=false`, overlay clicks are prevented
- **Press feedback:** On close button (mobile uses haptic feedback)

---

## Size Variants

| Size | Web (max-width) | Mobile |
|------|-----------------|--------|
| `sm` | `max-w-sm` | Full width, auto height |
| `md` | `max-w-md` (default) | Full width, auto height |
| `lg` | `max-w-lg` | Full width, auto height |
| `xl` | `max-w-xl` | Full width, auto height |
| `full` | `max-w-4xl` | Full width, maxHeight 80-90% |

Mobile uses fixed padding and `maxHeight: '90%'` with scroll if necessary.
...
| Token | Web | Mobile |
|-------|-----|--------|
| Background | `bg-card` | `#FFFFFF` (light-first) |
| Foreground | `text-foreground` | `colors.slate[900]` |
| Border | `border-border` | `colors.slate[200]` |
| Overlay | Dialog default | `rgba(15, 23, 42, 0.5)` (#0F172A @ 50%) |
| Icon | - | `colors.slate[500]` (#64748B) |
| Handle | - | `colors.slate[300]` |
...
**Web:**
- Relies on shadcn Dialog semantics (Radix UI)
- `aria-labelledby` pointing to title via DialogTitle
- `aria-describedby` pointing to description via DialogDescription
- Focus trapping within modal
- Escape key closes modal
- Overlay closes only when `closeOnOverlay=true`

**Mobile:**
- `accessibilityRole="button"` for close button
- `accessibilityLabel="Close modal"` for close button
- `accessibilityRole="header"` for title
- `onRequestClose` wired to `onClose` (supports Android hardware back)
...
- `cn` utility
- Optional `cva` (class-variance-authority)

**Mobile:**
- `Modal` from react-native
- `Pressable`, `View`, `Text`, `KeyboardAvoidingView`, `Platform`, `StyleSheet` from react-native
...
- Web tests/implementation independent of mobile; can execute in parallel
...
- Titles/descriptions appear with correct roles
- Footer slot renders
- Size applies correct max-width class
- Overlay click respects `closeOnOverlay` (prevents close when false)
- `onClose` invoked when dialog closes (close button, overlay, Escape)
- `DialogContent` has role `dialog`
- aria-labelledby and aria-describedby present
- Custom className merges correctly
- Handles rapid open/close without error

**Mobile (jest-expo + RNTL):**
- Renders children when open; does not render when closed
- Overlay press triggers `onClose` when allowed
- Title/description render with accessibility roles
- Close button press triggers `onClose` with haptic feedback
- `onRequestClose` fires (Android back button)
- Content respects safe area padding
- Handle indicator renders
...
- [ ] Accessibility attributes included (role, aria-label, accessibilityRole)
- [ ] Haptic feedback on mobile close actions
- [ ] Safe area insets respected on mobile
- [ ] Handle indicator renders on mobile
