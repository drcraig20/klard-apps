...
**Goal:** Build a production-ready AppSidebar navigation component for the Klard web application using shadcn/ui primitives with collapsible/off-canvas behavior, active item styling, badges/icons, keyboard accessibility, and Klard design tokens.

**Architecture:** Web uses shadcn/ui Sidebar with Radix UI primitives. The component provides collapsible navigation with icons, labels, badges, and active state highlighting. Includes a Skeleton subcomponent for loading states. Integrates with Next.js App Router via `next/link`.

**Tech Stack:**
- Web: Next.js 16 App Router, React 19, TypeScript strict, Tailwind CSS 4, shadcn/ui Sidebar, class-variance-authority, lucide-react icons
...
- **Group Label**: Section title for navigation items (default: "Application")
- **Menu Items**: Clickable navigation links with icons
- **Badges**: Optional notification counts (small pill)
- **Footer**: Optional bottom content area
- **Trigger**: Mobile toggle button via `SidebarTrigger`
...
| Border | border-border token | border-border token |
| Focus Ring | ring-primary/50 | ring-primary/50 |

**Visual Tokens:** Spacing 12–16px; border-radius 12px.

---

## Accessibility Requirements

- Nav landmarks (`<nav>` role or semantic `<nav>`)
- `aria-current="page"` on active links
- Focus-visible outlines (`focus-visible:ring-2`)
- Keyboard tab order preserved
- Trigger is a button element
- Offcanvas should trap focus via shadcn defaults
...
|-----------|-----|-----|-----|-----|-----|
| AppSidebar | Render nav from data + handle collapse/active styles | Extendable via items/header/footer/className without code changes | Works for any item set matching contract | Props limited to sidebar concerns | Depends on shadcn primitives and injected data |
| AppSidebar.Skeleton | Display loading placeholders | Extendable by count prop | Substitutable wherever menu expected | Only uses count prop | Depends on skeleton primitives |
...
- Existing `Badge` component for counts (optional)
...
**Step 1: Add active state, accessibility, and badge tests**
...
## Task 4: Write Failing Tests for Header, Footer, Group Label, and Trigger

**Files:**
- Modify: `klard-web/src/__tests__/components/ui/app-sidebar.test.tsx`

**Step 1: Add header, footer, group label, and trigger tests**
...
## Task 5: Write Failing Tests for Skeleton

**Files:**
- Modify: `klard-web/src/__tests__/components/ui/app-sidebar.test.tsx`

**Step 1: Add skeleton tests**
...
Expected: FAIL - Skeleton not implemented

---

## Task 6: Implement AppSidebar Component

**Files:**
- Create: `klard-web/src/components/ui/app-sidebar.tsx`

**Step 1: Create the AppSidebar component with Skeleton**
...
export interface AppSidebarProps {
  ...
  activeItem?: string;
  /** Optional group label (default: "Application") */
  groupLabel?: string;
...
/**
 * Props for AppSidebar.Skeleton component
 */
 ...
 * - Active item highlighting with Klard teal color and aria-current
                  ...
                  return (
                    <SidebarMenuItem key={item.id} data-slot="menu-item">
                      ...
                      {item.badge !== undefined && item.badge > 0 && (
                        <SidebarMenuBadge
                          data-slot="badge"
                          className="bg-teal-600 text-white dark:bg-teal-500"
                        >
                          {item.badge}
                        </SidebarMenuBadge>
                      )}
...
/**
 * Skeleton loading state for AppSidebar
 */
function AppSidebarSkeleton({ count = 4 }: AppSidebarSkeletonProps) {
  return (
    <SidebarProvider>
      <Sidebar data-testid="app-sidebar-skeleton" data-slot="app-sidebar">
        <SidebarContent>
          <SidebarGroup>
            <SidebarGroupContent>
              <SidebarMenu>
                {Array.from({ length: count }).map((_, index) => (
                  <SidebarMenuItem key={index}>
                    <SidebarMenuSkeleton
                      showIcon
                      data-testid="sidebar-skeleton-item"
                    />
                  </SidebarMenuItem>
                ))}
              </SidebarMenu>
...
## Task 7: Export AppSidebar from Index
...
```typescript
export { AppSidebar, type AppSidebarProps, type SidebarNavItem, type AppSidebarSkeletonProps } from './app-sidebar';
```
...
- Add Skeleton subcomponent for loading states
...
- [ ] Active item has `data-active="true"` and `aria-current="page"`
- [ ] Active item has Klard teal styling (bg-teal-50, text-teal-700)
- [ ] Badges render when provided (count > 0)
- [ ] Badge uses Klard teal color
...
- [ ] Trigger button renders and toggles sidebar
- [ ] Focus-visible ring on menu items
- [ ] Hover states work (slate-100 light, slate-800 dark)
- [ ] TypeScript compiles with no errors
- [ ] All tests passing
- [ ] Exported from index.ts

**AppSidebar.Skeleton:**
- [ ] Renders default 4 skeleton items
- [ ] Renders specified count of skeleton items
- [ ] Shows icon placeholders

---

## Task 10: Mark Component as DONE
...
| 2 | Write failing tests - Rendering |
| 3 | Write failing tests - Active State & Badges |
| 4 | Write failing tests - Header, Footer, Group Label, Trigger |
| 5 | Write failing tests - Skeleton |
| 6 | Implement AppSidebar component with Skeleton |
| 7 | Export from index.ts |
| 8 | Run full test suite |
| 9 | Verification checklist |
| 10 | Mark component as DONE |
...
│   ├── app-sidebar.tsx    # Klard component with Skeleton
...
**Plan complete and saved to `docs/screens/comp-plan/3.5-sidebar-implementation-plan-merged.md`.**
