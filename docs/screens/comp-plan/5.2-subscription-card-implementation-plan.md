# 5.2 Subscription Card Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a production-ready SubscriptionCard component for both Web (Next.js + shadcn/ui) and Mobile (React Native + Expo) that displays subscription summary information including service logo, name, price, next renewal date, and status.

**Architecture:** Web uses shadcn Card with Avatar and Badge components, following existing patterns. Mobile uses React Native View with expo-image for service logo and native styling. Both support variants for different display contexts (list, compact, detailed).

**Tech Stack:**
- Web: Next.js 16, React 19, TypeScript strict, Tailwind CSS 4, lucide-react, CVA
- Mobile: React Native, Expo SDK 54, TypeScript strict, expo-image, expo-haptics
...
```typescript
// Shared subscription data type (from commons)
...
|-----------|----------------|
| **SRP** | SubscriptionCard only renders subscription summary - no data fetching |
| **OCP** | Variants via props, status colors via config mapping |
| **LSP** | All subscription statuses produce valid cards |
| **ISP** | Minimal interface with only display-related props |
| **DIP** | Depends on SubscriptionData type abstraction, not concrete API |
...
**Web:**
- `Avatar` from `@/components/ui/avatar` - for service logo display
- `Badge` from `@/components/ui/badge` - for status display
- `Card` from `@/components/ui/card` - base container (optional, may use simple div)
- `cn` from `@/lib/utils` - class merging

**Mobile:**
- `Badge` from `@/components/ui/Badge` - for status display
- `expo-image` - for service logo with caching
...
### Mobile
- `expo-image` - Service logo display
...
```bash
git add klard-web/src/components/ui/subscription-card.tsx klard-web/src/components/ui/index.ts klard-web/src/__tests__/components/ui/subscription-card.test.tsx
...
- Include service logo with fallback
...
```bash
git add klard-mobile/src/components/ui/SubscriptionCard.tsx klard-mobile/src/components/ui/index.ts klard-mobile/src/__tests__/components/ui/SubscriptionCard.test.tsx
...
- [ ] Service logo with fallback works
- [ ] onPress interaction functional
- [ ] Custom className merges correctly
- [ ] Accessibility: role="button", aria-label when clickable
...
- [ ] All 3 variants functional (default, compact, detailed)
- [ ] Status badges display correct colors
- [ ] expo-image logo with fallback works
- [ ] Haptic feedback on press
- [ ] TypeScript compiles with no errors
- [ ] All tests passing
- [ ] Exported from index.ts
...
| 5 | Web | Implement SubscriptionCard component (TDD GREEN phase) |
| 6 | Web | Export from index.ts |
| 7-9 | Mobile | Write failing tests (TDD RED phase) |
| 10 | Mobile | Implement SubscriptionCard component (TDD GREEN phase) |
...
**Plan complete and saved to `docs/screens/comp-plan/5.2-subscription-card-implementation-plan.md`.**
