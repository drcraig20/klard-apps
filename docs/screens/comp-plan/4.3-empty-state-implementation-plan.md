...
**Architecture:** Compound component pattern for web (EmptyState + sub-components), single component with props for mobile. Both platforms support illustrations, title, description, and action buttons with consistent Klard design system styling.

**Tech Stack:**
- Web: React 19, TypeScript, Tailwind CSS 4, CVA for variants
- Mobile: React Native, Expo SDK 54, expo-image, StyleSheet
...
**SRP:** Each sub-component handles ONE visual concern
**OCP:** Extend via ReactNode children/slots, not modification
**LSP:** N/A (no inheritance hierarchy)
**ISP:** Separate props interfaces per sub-component
**DIP:** Components depend on Button abstraction, not concrete implementation
...
- Task 1 (Web tests) + Task 5 (Mobile tests) - No file conflicts
- Task 2 (Web impl) + Task 6 (Mobile impl) - Different packages
- Task 3 (Web exports) + Task 7 (Mobile exports) - Different index files
...
describe("EmptyState", () => {
  ...
  describe("Sizes", () => {
    it("should apply sm size", () => {
      ...
      expect(container?.className).toMatch(/py-6|p-6/)
    })
    ...
    it("should apply md size by default", () => {
      ...
      const container = document.querySelector('[data-slot="empty-state"]')
      expect(container?.className).toMatch(/py-12|p-12/)
    })
    ...
    it("should apply lg size", () => {
      ...
      const container = document.querySelector('[data-slot="empty-state"]')
      expect(container?.className).toMatch(/py-16|p-16/)
    })
  })
...
**Step 3: Create placeholder illustration assets**

Create simple placeholder files (can be replaced with real assets later):

```bash
# Create empty placeholder images if they don't exist
mkdir -p klard-mobile/assets/images
# Touch the files - actual images should be added by designer
```
