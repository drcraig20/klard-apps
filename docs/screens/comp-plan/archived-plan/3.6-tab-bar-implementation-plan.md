...
**Goal:** Build a reusable mobile TabBar for bottom navigation with icon/label support, optional badges, center action button (FAB) styling, safe-area padding, haptic feedback, and active/inactive states consistent with the Klard design system.

**Architecture:** The TabBar is a standalone reusable UI component that renders bottom navigation tabs. It uses `useSafeAreaInsets` for notch/home indicator handling, `expo-haptics` for tactile feedback, and supports flexible tab configurations including badges and a floating center action button. The component follows existing Badge/Button patterns in the codebase. Keep logic presentation-only; navigation handled by parent via `onTabPress`. Styling via `StyleSheet` to match Klard tokens (primary teal, muted text, card background).

**Tech Stack:** React Native, Expo SDK 54, expo-haptics, react-native-safe-area-context, @expo/vector-icons (Ionicons), jest-expo + React Native Testing Library for tests.
...
| Active | Selected tab with primary color, top indicator/glow |
| With Badge | Tab displays notification count pill |
| Center Button | Floating action button style (elevated, shadow/glow) |
| Safe Area | Bottom padding for home indicator via `useSafeAreaInsets` |
...
|-------|-------------|------------|-------|
| Primary | `#0D7C7A` | `#15B5B0` | Active tab icon/label |
| Muted | `#64748B` | `#94A3B8` | Inactive tab icon/label |
| Background | `#FFFFFF` | card/surface | Tab bar background |
| Border Top | `#E2E8F0` | `#CBD5E1` | Container border |
| Center Button BG | `#0D7C7A` | `#15B5B0` | FAB background |
| Center Button Icon | `#FFFFFF` | `#FFFFFF` | FAB icon |
| Badge BG | `#DC2626` | `#EF4444` | Badge background |
| Badge Text | `#FFFFFF` | `#FFFFFF` | Badge text |
...
- Tab items: `accessibilityRole="tab"`, `accessibilityState={{ selected: isActive }}`
- Center button: `accessibilityRole="button"`, `accessibilityLabel` with action name
- Badge announced by including count in `accessibilityLabel` (e.g., "Alerts, 5 notifications")
- Minimum touch target 44px, hitSlop added for small targets
...
1. **Rendering tests** - Component renders, tabs display, accessibility roles
2. **Active state tests** - Active highlight, inactive state, selected accessibility
3. **Interaction tests** - Tab press calls handler, haptic feedback triggers
4. **Badge tests** - Badge display, zero/undefined hidden, 99+ overflow
5. **Center button tests** - FAB style, no label, medium haptic
6. **Edge cases** - Empty tabs array (if supported)
        ...
        <Pressable
          ...
          accessibilityLabel={
            tab.badge && tab.badge > 0
              ? `${tab.name}, ${tab.badge} notifications`
              : tab.name
          }
        >
...
- [ ] Accessibility attributes included (tablist role, tab roles, selected state, badge in label)
