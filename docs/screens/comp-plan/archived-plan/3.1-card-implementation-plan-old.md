# 3.1 Card Component Implementation Plan
...
**Architecture:** Web creates a KlardCard wrapper component using CVA (class-variance-authority) for variant styling. Mobile creates a Card component using View/Pressable with StyleSheet. Both support variants (default, elevated, ghost, interactive), padding options, and optional press handling.
...
| Variant | Web Classes | Mobile Style |
|---------|-------------|--------------|
| default | `border border-slate-200` | `borderWidth: 1, borderColor: #E2E8F0` |
| elevated | `shadow-md border-0` | `elevation: 4, shadowOpacity: 0.1` |
| ghost | `bg-transparent border-0` | `backgroundColor: transparent` |
| interactive | `border hover:border-teal-500 hover:shadow-md cursor-pointer` | `pressable with scale feedback` |
