...
**Architecture:** Presentational components per platform with no data fetching; gradients keyed off status; size tokens drive spacing/typography; shared formatting helpers for spend progress and expiry. Web uses Tailwind CSS gradients; mobile uses `expo-linear-gradient` plus RN primitives.

**Tech Stack:** Next.js 16, React 19, Tailwind CSS 4, class-variance-authority patterns, Vitest + RTL; Expo SDK 54, `expo-linear-gradient`, React Native Testing Library + Jest.
...
- **Card Number**: Masked with last 4 digits visible (•••• •••• •••• XXXX)
...
- **Progress Bar**: Spending progress at bottom (capped at 100%)
...
| Size | Web Classes | Mobile (px) | Padding | Use Case |
|------|-------------|-------------|---------|----------|
| sm | `w-64 h-40` | 256×160 | 12px | List items, compact views |
| md | `w-80 h-48` | 320×192 | 16px | Default card display |
| lg | `w-96 h-56` | 384×224 | 20px | Featured/detail views |
...
|-----------|----------------|
| **SRP** | BurnerCardVisual only renders card visual - no data fetching, no business logic |
| **OCP** | Extended via className/style props, status variants via config map |
| **LSP** | All status/size combinations produce valid cards with consistent interface |
| **ISP** | Minimal interface with only necessary props (card data + optional size/style) |
| **DIP** | Component depends on abstractions (theme colors, gradient config, formatting utils) |
...
**Plan complete and saved to `docs/screens/comp-plan/5.3-burner-card-visual-implementation-plan-merged.md`.**
