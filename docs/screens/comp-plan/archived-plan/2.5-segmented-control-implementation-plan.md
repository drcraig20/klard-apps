...
**Architecture:** Web uses shadcn/ui `Tabs` components styled as a segmented control with CVA-driven variants for size/full-width, data-slot tagging, and icon+label layout. Mobile uses platform-native styling with a React Native Pressable-based implementation, expo-haptics for feedback, and a thin prop adapter for value/index mapping. State is controlled via `value`/`onChange`.

**Tech Stack:** Next.js 16 App Router, React 19, Tailwind 4 + shadcn/ui Tabs, class-variance-authority, Vitest + Testing Library (web); Expo SDK 54, expo-haptics, React Native Testing Library (mobile).
...
- **Targets:** Web (primary), Mobile (secondary). Web first per guidance.
- **Visual states:** default, hover (TabsTrigger hover), active (data-state=active), focus-visible ring, disabled (opacity reduced), error N/A, loading N/A.
...
**ISP:** Interface contains only necessary props (value, onChange, options, size, fullWidth, disabled)
...
|------|--------|---------|-----------|
| sm | 32px (h-8) | px-2 | text-sm (13px mobile) |
| md | 40px (h-10) | px-3 | text-sm (14px mobile) |
...
|-------|------------|-----------|-------|
| Background | `#F1F5F9` (slate-100) | `#1E293B` (slate-800) | Container bg |
| Selected Bg | `#FFFFFF` | `#0F172A` (slate-950) | Active segment |
| Selected Text | `#0D7C7A` (teal-700) | `#15B5B0` (teal-400) | Active label |
| Default Text | `#64748B` (slate-500) | `#94A3B8` (slate-400) | Inactive label |
| Disabled | opacity 0.5 | opacity 0.5 | Disabled state |
