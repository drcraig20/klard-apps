...
**Architecture:** Stateless, controlled display-only component. Web uses a client component with Intl.NumberFormat, size variants via CVA, and optional change indicator with icons/colors from the design system. Mobile uses a React Native functional component with StyleSheet-driven sizes/colors and the same props surface. Both expose minimal logic (formatting + delta computation) and rely on props for data, keeping SRP.

**Tech Stack:**
- Web: Next.js 16, React 19, Tailwind CSS 4, CVA, lucide-react icons, TypeScript strict mode
- Mobile: Expo 54, React Native 0.81, StyleSheet, @expo/vector-icons (Ionicons), TypeScript strict mode
- Shared: Intl.NumberFormat for currency formatting
...
- **Visual states:**
  ...
  - Billing cycle suffix (monthly/yearly/weekly/one-time)
  - Change indicator with up/down icon and colored text
  - Size variants (sm/md/lg)
  - Resilient to large numbers with thousand separators
  - Handles zero/negative amounts gracefully
- **Size variants (Web CVA):**
  - `sm`: text-sm
...
- **Color tokens:**
  - Primary/foreground text: `text-slate-900 dark:text-slate-100`
  - Muted suffix: `text-slate-500 dark:text-slate-400`
  - Increase (bad): `text-red-500 dark:text-red-400`
  - Decrease (good): `text-green-500 dark:text-green-400`
  - Background: transparent (inline element)
- **Accessibility requirements:**
  - Semantic inline text elements
  - Icons have `aria-hidden="true"`
  - Provide `aria-label` or `accessibilityLabel` for change description (e.g., "$12.00 per month, increased by $3.00")
  - Avoid hard-coded locale text
  - Use `accessibilityRole="text"` on mobile container
- **Dependencies:**
  - Web: existing `cn` helper, lucide-react (ArrowUp, ArrowDown) already installed
...
- **Test strategy (TDD - write first):**
  - **Basic Rendering**: formatted amount, data-slot attribute, decimal formatting, zero handling, thousand separators
  - **Currency Support**: USD default, EUR, GBP formatting
  - **Size Variants**: sm/md/lg className or style application
  - **Billing Cycle Labels**: monthly (/mo), yearly (/yr), weekly (/wk), one-time (none)
  - **Price Change Indicators**: increase (red + up arrow), decrease (green + down arrow), delta calculation
  - **Custom className**: merge with default classes
  - **Edge Cases**: negative amounts, very small amounts
...
- [ ] All size variants functional (sm/md/lg)
- [ ] All billing cycle labels working (monthly/yearly/weekly/one-time)
- [ ] Price change indicators show correct colors (red=increase, green=decrease)
- [ ] Currency formatting with Intl.NumberFormat working
- [ ] Matches design specification visually
- [ ] Accessibility attributes included (aria-hidden, accessibilityRole, accessibilityLabel)
- [ ] TypeScript passes with no errors
- [ ] All tests written first (TDD) and passing
- [ ] Works on both platforms (Web + Mobile)
- [ ] Exported from components/ui/index.ts
- [ ] Specification marked as DONE
...
|------|-------------|----------|
| 1 | Write basic rendering tests (RED) | Web |
| 2 | Write size and billing cycle tests (RED) | Web |
| 3 | Write price change indicator tests (RED) | Web |
| 4 | Implement PriceDisplay component (GREEN) | Web |
| 5 | Export from index + commit | Web |
| 6 | Write all tests (RED) | Mobile |
| 7 | Implement PriceDisplay component (GREEN) | Mobile |
