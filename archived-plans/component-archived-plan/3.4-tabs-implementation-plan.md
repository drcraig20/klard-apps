...
**Architecture:** Web uses shadcn/ui `Tabs` primitives (built on Radix UI) to render a Tabs container with a `TabsList`/`TabsTrigger` map and consumer-supplied `TabsContent`. Style variants handled with Tailwind + CVA for size/fullWidth variations, with data-slot tagging and badge/icon layout. Mobile uses a horizontal ScrollView-based tab bar with Pressable tabs, optional badges/icons, safe area-aware padding, haptic feedback, and controlled selection.

**Tech Stack:**
- Web: Next.js 16 App Router, React 19, Tailwind 4, shadcn/ui Tabs (Radix UI), Badge, class-variance-authority, `@/lib/utils` cn helper, Vitest + Testing Library
- Mobile: Expo SDK 54, React Native 0.81, ScrollView/Pressable/Text/View, `expo-haptics` (for selection feedback), React Native Testing Library
...
| State | Description |
|-------|-------------|
| Default | Base trigger styling with slate text |
| Hover | Subtle background change on trigger hover |
| Active | `data-state=active` with white background, primary text, subtle shadow/border |
| Focus-visible | Focus ring `ring-2 ring-primary/50` |
| Disabled | Reduced opacity, non-interactive |
| Badge visible | Badge renders when `badge` prop provided |
...
| Size | Description |
|------|-------------|
| `md` (default) | Standard padding/height matching shadcn TabsList |
| `sm` | Reduced height, gap, and padding |

Implementation via CVA on trigger component.
...
|-------|-----------|-----------|-------|
| TabsList bg | `bg-slate-100` | `bg-slate-800` | List container background |
| Active trigger bg | `bg-white` | `bg-slate-700` | Selected tab background |
| Active text | `text-primary` (#0D7C7A) | `text-primary` (#15B5B0) | Selected tab text |
| Inactive text | `text-slate-700` | `text-slate-300` | Unselected tab text |
| Focus ring | `ring-primary/50` | `ring-primary/50` | Focus indicator |
| Badge bg | `bg-slate-100` | `bg-slate-700` | Badge background |

---

## Accessibility

- Radix Tabs handles `role="tablist"`, `role="tab"`, `aria-selected`, keyboard navigation
- `data-slot="tabs"` on root container
- `data-slot="tabs.trigger"` per trigger
- `TabsContent` exported for consumer panels with `role="tabpanel"`
- Mobile: `accessibilityRole="tab"` and `accessibilityState={{ selected, disabled }}`
...
- `@/components/ui/badge` (for badge rendering)
- `@/lib/utils` cn helper
- `class-variance-authority` for size/fullWidth variants

**Mobile:**
- `expo-haptics` for selection feedback
- React Native core: ScrollView, Pressable, Text, View, StyleSheet
...
git commit -m "feat(web): implement TabsContainer component with badges, icons, and size variants"
```
