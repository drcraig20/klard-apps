...
**Architecture:** Web wraps shadcn/ui Input with search-specific affordances (search icon, clear button, loading spinner, debounced callback). Mobile uses React Native TextInput with Ionicons and custom debounce implementation. Both support controlled value, immediate onChange, and debounced onSearch. Encapsulate icons and stateful behaviors (loading, clear) within the component while keeping data/control outside (SRP).

**Tech Stack:**
- Web: Next.js 16, React 19, TypeScript strict, shadcn/ui Input, lucide-react, Tailwind CSS 4
- Mobile: React Native, Expo SDK 54, TypeScript strict, @expo/vector-icons (Ionicons), ActivityIndicator
- No external debounce library needed (self-contained via useEffect/setTimeout)
...
Single default size (YAGNI - no additional sizes):
- **Web:** height 40px (`h-10`), padding `pl-10 pr-10`
- **Mobile:** height 48px, horizontal padding 12px
...
- `role="search"` wrapper (web)
- `type="search"` on input (web)
- `aria-label` / `accessibilityLabel` on clear button: "Clear search"
- `accessibilityRole="button"` on clear button (mobile)
- Focus-visible rings
- Maintain tab order
- `returnKeyType="search"` (mobile)
- Loading spinner is decorative (no aria-live needed)
...
|-----------|----------------|
| **SRP** | SearchInput only handles search input UI - no business logic; encapsulates icons and stateful behaviors |
| **OCP** | Extensible via className/style props, debounce timing configurable via prop |
| **LSP** | Handles all standard input props via spread; works with any onChange/onSearch callbacks |
| **ISP** | Minimal interface with only search-specific props; no unrelated behaviors |
| **DIP** | Web depends on Input abstraction, Mobile depends on TextInput; callbacks injected |
...
- No external debounce library (self-contained)
...
## Task 1: Write Failing Tests for SearchInput Rendering
...
## Task 2: Write Failing Tests for Clear Button
...
## Task 3: Write Failing Tests for Loading State and Debounce
...
## Task 4: Implement SearchInput Component
...
## Task 5: Export SearchInput from Index
...
```bash
git add klard-web/src/components/ui/search-input.tsx klard-web/src/components/ui/index.ts klard-web/src/__tests__/components/ui/search-input.test.tsx
...
- Self-contained debounce (no external deps)
...
## Task 6: Write Failing Tests for Mobile SearchInput Rendering
...
## Task 7: Write Failing Tests for Mobile Clear Button
...
## Task 8: Write Failing Tests for Mobile Loading and Debounce
...
## Task 9: Implement Mobile SearchInput Component
...
## Task 10: Export Mobile SearchInput from Index
...
| 4 | Web | Implement SearchInput component (TDD GREEN phase) |
| 5 | Web | Export from index.ts |
| 6-8 | Mobile | Write failing tests (TDD RED phase) |
| 9 | Mobile | Implement SearchInput component (TDD GREEN phase) |
...
**Plan complete and saved to `docs/screens/comp-plan/2.4-search-input-implementation-plan-merged.md`.**
